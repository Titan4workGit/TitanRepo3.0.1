...
app.controller('myCtrl', function($scope,$http) 
{
	 ...
	 $scope.imageUrl="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo3.0.1@latest/SiteAssets/EmployeeSynchronous/EmployeeDirectory/user_pic.jpg";
			...
			{				
			  var ProjectManager=getPeopleUserInfoGroups('ProjectManagerPicker');
			    if(ProjectManager.length==0){
                    var terminateProject=document.getElementById("terminate_projects").checked;
                    if(terminateProject==false){
                        alert('Please Enter live project reassigned to Project Mamber Name!');
                        return false;
                    }
			    }
			    var reassignedManager=getPeopleUserInfoGroups('subordinatesManagerPicker');
			    if(reassignedManager.length==0){
                    var managerSkip=document.getElementById("skip_fornow").checked;
                    if(managerSkip==false)
                    {
                        alert('Please Enter manager name to reassigned  manager!');
                        return false;
                    }			      
			    }
			    var Delegated_UserId=getPeopleUserInfoGroups('managerPicker');
			    if(Delegated_UserId.length==0){
                    var documentskip=document.getElementById("skip_documentsec").checked;
                    if(documentskip==false)
                    {
                        alert('Please Enter personal documents reassigned to Mamber Name!');
                        return false;
                    }
			    }
              getLiveProject($scope.LogonNameId);
              if(reassignedManager.length>0){
                getEmployeeByManagerId($scope.LogonNameId);
              }
			  $scope.updateTerminationInformation();
			  
			  if(documentskip==true){
...
$scope.GetEmployeeTeamList = function ()
	...
	{
		var attachmentUrl=_spPageContextInfo.webAbsoluteUrl + '/_layouts/15/userphoto.aspx?accountname=' + escapeProperly(EMail)    //"https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo3.0.1@latest/SiteAssets/EmployeeSynchronous/EmployeeDirectory/user_pic.jpg";
...
function addDMS_Access_Delegation(UserEMail,UserId){   
    ...
    var DMS_Link= _spPageContextInfo.webAbsoluteUrl+ "/DocumentManagementSystem/Forms/AllItems.aspx";
...
//#region get Project by managerId
var getEmployeeByManagerId= (userId) =>{
    var query=_spPageContextInfo.webAbsoluteUrl +"/_api/web/lists/getbyTitle('Employees')/Items/?$select=*&$filter= ManagerLoginNameId eq "+userId+" and Status eq 'Active'";
    $.ajax({
        url:query,
        method:'get',
        headers:{accept: "application/json;odata=verbose"},
        ...
        success :function(data)
        {
            ...
            if(results.length>0){
                for (let index = 0; index < results.length; index++) {                    
                    var Metadata='';                    
                    var ItemType = GetItemTypeForListName('Employees');
                    //var terminateProject=document.getElementById("terminate_projects").checked;
                        var ProjectManager=getPeopleUserInfoGroups('subordinatesManagerPicker');
                        var ParentId=getParentIdByManagerEmail(ProjectManager[0]);
                        Metadata = {
                            __metadata: {
                                'type': ItemType
                            },
                            ManagerLoginNameId:ProjectManager[0],
                            ParentId:ParentId
                        }
                                
                    var ListName='Employees';
                    updateUserStatus(Metadata,results[index].Id,ListName);
                }
...
//#region  get all live Project 

var getLiveProject= (userId) =>{
    var query=_spPageContextInfo.webAbsoluteUrl +"/_api/web/lists/getbyTitle('ProjectDetails')/Items/?$select=*&$filter= ManagerNameId eq "+userId+" and Status eq 'Live'";
    $.ajax({
        url:query,
        method:'get',
        headers:{accept: "application/json;odata=verbose"},
                ...
                for (let index = 0; index < results.length; index++) {                    
                    ...
                    if(terminateProject==true){
                        ...
                        Metadata = {
                            ...
                            ManagerNameId:ProjectManager[0]
                        }
                    }                 
...
//#region  updated ProjectTeamDetails user Status

var updateUserStatus = (Metadata,EditId,ListName) =>{
    
    var ownURL = _spPageContextInfo.webAbsoluteUrl + "/_api/web/Lists/GetByTitle('"+ListName+"')/Items(" + EditId + ")";
    $.ajax({
        url: ownURL,
        type: "POST",
        async: false,
...
var getParentIdByManagerEmail = (managerId) =>{
var Query = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Employees')/items/?$select=Id&$filter=LogonNameId eq '" + managerId+ "'";
var parentId=-1;
$.ajax({
        url:Query,
        method:'get',
