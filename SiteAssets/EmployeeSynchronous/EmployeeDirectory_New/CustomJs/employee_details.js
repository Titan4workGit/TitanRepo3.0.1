...
app.controller('myCtrl', function ($scope, $http, $window) {
    ...
    sessionStorage.setItem('g_Item_Id',employeedIddetails);
    ...
    $scope.imageUrl = "https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo3.0.1@latest/SiteAssets/EmployeeSynchronous/EmployeeDirectory_New/images/user_pic.jpg";
    ...
    $scope.GetEmployees = function () {
        $http({
            ...
            if (response.data.d.results.length > 0) {
                ...
                if(_spPageContextInfo.userEmail.toLowerCase()==response.data.d.results[0].Email)
                {
                   if(IsHRAdmin==true){
                      $('#spnUploadimgbx').show();
                      $('.lblChangeImageByHr').hide();
                   }                   
                }
                else{
                     $('#spnUploadimgbx').hide();
                        ...
                        if (AnnouncementData.length > 0) {
                            ...
                            if (days <= 30) {
                                ...
                                $("#NotifyAnnoucement").click(function () {
                                    if ($("#postInTimeLine").prop("checked") || $("#Corporatesec").prop("checked") || $("#Companysec").prop("checked") || $("#Departmentsec").prop("checked") || $("#locationsec").prop("checked") || $("#nosenddata").prop("checked")) {
                                        if ($("#postInTimeLine").prop("checked")) {
            ...
            $.when($scope.AddItemToList(ListName, Metadata)).done(function (MainExamListItemTemp) {
                ...
                if (fileImageexist.length > 0) {
                    ...
                    if (days <= 30) {
                        ...
                        $("#addsuccessful").modal('show');
                        $("#NotifyAnnoucement").click(function () {
                            if ($("#Corporatesec").prop("checked") || $("#postInTimeLine").prop("checked") || $("#Companysec").prop("checked") || $("#Departmentsec").prop("checked") || $("#locationsec").prop("checked") || $("#nosenddata").prop("checked")) {
                                if ($("#postInTimeLine").prop("checked")) {
                                	$("#NotifyAnnoucement").attr('disabled', 'disabled');
    ...
    $scope.GetAttachment = function (attachmentcollection, EMail) {
        var attachmentUrl = _spPageContextInfo.webAbsoluteUrl + '/_layouts/15/userphoto.aspx?accountname=' + escapeProperly(EMail) //"https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo3.0.1@latest/SiteAssets/EmployeeSynchronous/EmployeeDirectory_New/images/user_pic.jpg";
        if (attachmentcollection.results.length > 0) {
            attachmentUrl = attachmentcollection.results[0].ServerRelativeUrl;
            sessionStorage.setItem('alreadyExistUserPick',true);
            var FileName=attachmentcollection.results[0].FileName;
            sessionStorage.setItem('g_files_Name',FileName);
            $scope.teamporaryImageUrl = attachmentcollection.results[0].ServerRelativeUrl
        }
        else{sessionStorage.setItem('alreadyExistUserPick',false);}
...
var checkedAudience = 'Corporate';
$(document).ready(function () {
    //CheckTFWEmp();   
    
    $('#spnUploadimgbx').click(function (){
          $('#uploadimgbx').modal('show');
          //$('#spnUploadimgbx').hide();
    })
    $('#btnUserPhotoClose').click(function (){
         if(sessionStorage.getItem('alreadyExistUserPick')=='true'){
             $('#profile-image1').attr('src', _spPageContextInfo.webAbsoluteUrl + '/_layouts/15/userphoto.aspx?accountname=' + escapeProperly(_spPageContextInfo.userEmail))
         }
    })
    
    checkHR_Admin();
    timeLineCheckBoxBind();
    ...
    $("#uploadFile1").on('change', function () {
        if (typeof ($("#uploadFile1")[0].files) != "undefined") {
            ...
            if (size <= 100) {
                ...
                $("#uploadFile1").val(null);
                $('#profile-image1').attr("src", "https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo3.0.1@latest/SiteAssets/EmployeeSynchronous/EmployeeDirectory_New/images/user_pic.jpg");
                return false;
            }
        }
    });
...
function InsertWelcomeInAnnouncementList(_ItemID) {
    ...
    metadataDescription["IsFlagUp"] = IsFlagUp
...
var timeLineCheckBoxBind = () =>{
    $('#nosenddata').change(function () {
        if(this.checked){
            $('#postInTimeLine').prop('checked',true)
            $('#postInTimeLine').prop('disabled',true);
        }
        else{
            $('#postInTimeLine').prop('disabled',false)
        }    
    });
    $('#postInTimeLine').change(function () {
        if($("#nosenddata").prop("checked")){
            $('#postInTimeLine').prop('checked',true)
            $('#postInTimeLine').prop('disabled',true)
            //alert("You can't uncheck 'Post in the Timeline', if you have checked 'Send Notification'!");
        }
    });
}
